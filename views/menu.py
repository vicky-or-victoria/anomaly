...
def build_contract_detail_embed(theme: dict, c) -> discord.Embed:
    ...
    if cap > 0:
        embed.add_field(
            ...
            value=(
                ...
                else f"**{slots_left}** slot(s) open — use **🚀 Deploy Roster** to deploy."
            ),
            inline=False,
        )
    else:
        embed.add_field(
            name="Deployment",
            value="Awaiting GM activation — deployment opens once the GM sets this contract up.",
            inline=False,
        )
...
def build_contract_board_embed(theme: dict, rows) -> discord.Embed:
    ...
    for c in rows:
        ...
        lines.append(
            ...
            f"  `{c['status'].replace('_',' ').title()}`  ·  {dep}/{cap} deployed"
        )
...
class ContractSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        detail_embed = build_contract_detail_embed(theme, c)
        await interaction.response.send_message(
            embed=detail_embed,
            view=ContractActionView(interaction.guild_id, selected_id),
            ephemeral=True,
        )
...
async def build_menu_embed(guild_id: int, conn, theme: dict = None) -> discord.Embed:
    ...
    cfg        = await conn.fetchrow(
        "SELECT turn_interval_hours, contract_name, operational_tempo, tempo_threshold FROM guild_config WHERE guild_id=$1",
        guild_id)
